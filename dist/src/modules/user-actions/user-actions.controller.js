"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserActionsController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const user_actions_service_1 = require("./user-actions.service");
const create_action_dto_1 = require("./dto/create-action.dto");
const current_user_decorator_1 = require("../../common/decorators/current-user.decorator");
let UserActionsController = class UserActionsController {
    constructor(actionsService) {
        this.actionsService = actionsService;
    }
    createAction(userId, createActionDto) {
        return this.actionsService.createAction(userId, createActionDto);
    }
    getUserActions(userId, limit) {
        return this.actionsService.getUserActions(userId, limit);
    }
    getSelectionActions(selectionId) {
        return this.actionsService.getSelectionActions(selectionId);
    }
};
exports.UserActionsController = UserActionsController;
__decorate([
    (0, common_1.Post)(),
    (0, swagger_1.ApiOperation)({ summary: 'Log a care action' }),
    (0, swagger_1.ApiResponse)({ status: 201, description: 'Action logged successfully' }),
    __param(0, (0, current_user_decorator_1.CurrentUser)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, create_action_dto_1.CreateActionDto]),
    __metadata("design:returntype", void 0)
], UserActionsController.prototype, "createAction", null);
__decorate([
    (0, common_1.Get)('my-actions'),
    (0, swagger_1.ApiOperation)({ summary: 'Get user action history' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'List of user actions' }),
    __param(0, (0, current_user_decorator_1.CurrentUser)('id')),
    __param(1, (0, common_1.Query)('limit')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, Number]),
    __metadata("design:returntype", void 0)
], UserActionsController.prototype, "getUserActions", null);
__decorate([
    (0, common_1.Get)('selection/:selectionId'),
    (0, swagger_1.ApiOperation)({ summary: 'Get actions for specific selection' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'List of selection actions' }),
    __param(0, (0, common_1.Param)('selectionId', common_1.ParseIntPipe)),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", void 0)
], UserActionsController.prototype, "getSelectionActions", null);
exports.UserActionsController = UserActionsController = __decorate([
    (0, swagger_1.ApiTags)('User Actions'),
    (0, swagger_1.ApiBearerAuth)('JWT'),
    (0, common_1.Controller)('user-actions'),
    __metadata("design:paramtypes", [user_actions_service_1.UserActionsService])
], UserActionsController);
//# sourceMappingURL=user-actions.controller.js.map